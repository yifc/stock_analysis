...
USER_AGENTS = [
    "JobApplicationProject/1.0 (yfcoct@example.com)",
    "StockAnalysisTool/1.0 (yfcoct@example.com)",
    "FinancialResearchBot/1.0 (yfcoct@example.com)",
    "EducationalProject/1.0 (yfcoct@example.com)",
    "DataScraper/1.0 (yfcoct@example.com)"
]
...
class SEC10KTool(RagTool):
    ...
    def __init__(self, stock_name: Optional[str] = None, **kwargs):
        print("enter init")
        # Use Ollama embeddings (runs locally, no API key required)
    ...
    def _run(self, search_query: str, **kwargs: Any) -> Any:
        if 'stock_name' in kwargs:
            stock_name = kwargs.pop('stock_name')
            content = self.get_10k_url_content(stock_name)
            if content:
                self.add(content)
